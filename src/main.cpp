...
uint256 hashGenesisBlock("0xd3b778158671b4e989b8893e925fc19f74b61fa1428c797e06fd0627882bc0b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // H2Ocoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1395956886; // Line: 2815
...
const string strMessageMagic = "H2Ocoin Signed Message:\n";
...
{
    // H2Ocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 100 * COIN;
				...
				if(nHeight == 1)   
                        ...
                        nSubsidy = 200 * COIN;
				else if (nHeight > 2073600 && nHeight <= 12760800 )
						nSubsidy = 100 * COIN;
				else
						nSubsidy = 0 * COIN;
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // H2Ocoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // H2Ocoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // H2Ocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // H2Ocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3/27/2014 clean water for all";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 1478527;
        block.nTime = 1395956886;
        ...
        assert(block.hashMerkleRoot == uint256("0xae773ed4ffb9aadb4f1c29a796c9b8e950c08fde08ae7f24ef829ecbaa14b963"));
...
unsigned char pchMessageStart[4] = { 0xde, 0xfa, 0xce, 0xdd }; // H2Ocoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("H2OcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("H2OcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static H2OcoinMiner(CWallet *pwallet)
{
    printf("H2OcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("h2ocoin-miner");
    ...
    try { loop {
        ...
        printf("Running H2OcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
			...
			char oldscratchpad[SCRYPT_SCRATCHPAD_SIZE];
    ...
    {
        printf("H2OcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&H2OcoinMiner, pwallet));
}
